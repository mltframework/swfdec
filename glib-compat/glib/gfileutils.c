...
GReadFile *
g_readfile_new (const gchar *filename, GError      **error)
{
  GReadFile *readfile;
  int fd;
  int ret;
  ...
  readfile->length = st.st_size;

  readfile->contents = mmap (NULL, readfile->length, PROT_READ, MAP_PRIVATE,
      fd, 0);
  close (fd);
  if (readfile->contents != MAP_FAILED) {
    readfile->mmaped = TRUE;
  } else {
    readfile->mmaped = FALSE;
    ret = g_file_get_contents(filename, &readfile->contents, &readfile->length,
        error);
    if (!ret) {
      g_free (readfile);
      return NULL;
...
int
g_readfile_get_length (GReadFile *readfile)
{
...
void
g_readfile_free (GReadFile *readfile)
{
  if (readfile->mmaped) {
    munmap (readfile->contents, readfile->length);
  } else {
    g_free (readfile->contents);
  }
  g_free(readfile);
}
#endif
