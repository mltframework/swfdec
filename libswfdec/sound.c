...
void
swfdec_sound_free (SwfdecObject * object)
{
...
#ifdef HAVE_MAD
void
mp3_decode_mad (SwfdecObject * obj)
{
...
#ifndef HAVE_MAD
void
mp3_decode_mpglib (SwfdecObject * obj)
{
  MpglibDecoder *mp;
  int n;
  int offset = 0;
  ...
  while (ret == MPGLIB_OK) {
	...
	sound->sound_buf, sound->sound_len, &n);
  }
...
SwfdecSoundBuffer *
swfdec_sound_buffer_new (int len)
{
...
void
adpcm_decode (SwfdecDecoder * s, SwfdecObject * obj)
{
  bits_t *bits = &s->b;
  SwfdecSound *sound = obj->priv;
  int n_bits;
