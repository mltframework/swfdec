...
atomize_double (CompileState *state, jsdouble d)
{
  ...
  atom = js_AtomizeDouble (state->s->jscx, d, 0);
  ale = js_IndexAtom (state->s->jscx, atom, &state->atoms);
  if (ale == NULL) {
...
atomize_int32 (CompileState *state, int i)
{
  ...
  atom = js_AtomizeInt (state->s->jscx, i, 0);
  ale = js_IndexAtom (state->s->jscx, atom, &state->atoms);
  if (ale == NULL) {
...
{
  char *s = swfdec_bits_get_string (&state->s->b);
  push_string (state, s);
  g_free (s);
}
...
read_and_push_float (CompileState *state)
{
  ...
  double d = swfdec_bits_get_float (&state->s->b);
...
{
  guint8 command[3] = { JSOP_NUMBER, };
  double d = swfdec_bits_get_double (&state->s->b);
  jsatomid id = atomize_double (state, d);
...
read_and_push_int32 (CompileState *state)
{
  ...
  gint32 i = swfdec_bits_get_u32 (&state->s->b);
...
compile_wait_for_frame (CompileState *state, guint action, guint len)
{
  ...
  compile_state_add_action_jump (state, 
      swfdec_bits_get_u8 (&state->s->b), FALSE);
  compile_state_add_code (state, command, 3); \
}
...
{
  int amount = swfdec_bits_get_s16 (&state->s->b);
...
//#define SWFDEC_DUMP_SCRIPTS
...
#ifdef SWFDEC_DUMP_SCRIPTS
  {
    ...
    g_file_set_contents (filename, (char *) dump.ptr, s->b.ptr - dump.ptr, NULL);
    g_free (filename);
  }
#endif
