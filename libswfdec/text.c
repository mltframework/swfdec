...
void
swfdec_font_free (SwfdecObject * object)
{
...
void
swfdec_text_free (SwfdecObject * object)
{
...
SwfdecLayer *
swfdec_text_prerender (SwfdecDecoder * s, SwfdecSpriteSeg * seg,
    SwfdecObject * object, SwfdecLayer * oldlayer)
{
  ...
  for (i = 0; i < array->len; i++) {
    ...
    for (j = 0; j < text->glyphs->len; j++) {
      ...
      shape = g_ptr_array_index ((GPtrArray *) fontobj->priv, glyph->glyph);
	...
	art_affine_subpixel (trans);
      ...
      layervec->color = transform_color (text->color,
